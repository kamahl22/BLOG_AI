...
# Stat mapping for requested headers
STAT_MAPPING = [
    ("AB", "At Bats"),
    ("R", "Runs"),
    ("H", "Hits"),
...
player_id = 4142424  # Seiya Suzuki
season = 2025
team_id = '16'  # Cubs team ID
output_dir = "/Users/kamahl/BLOG_AI/sports/espn/mlb_test"
output_file = os.path.join(output_dir, f"player_{player_id}_{season}_gamelog.csv")
...
# Fetch data from API
logger.info(f"Fetching gamelog data for player ID {player_id}, season {season}")
...
# Check if seasonTypes exists (structured like in mlb_player_gamelog.py)
...
if events_dict:
    ...
    for game_id, event in events_dict.items():
        # Extract game data using gamelog_test.py method
        ...
        # Full team matchup
        team_name = 'Cubs'  # Default for this example
        ...
        # Get stats 
        stats = []
        # First try to get stats from event directly
        if 'stats' in event:
            stats = event.get('stats', [])
        else:
            # Try to look for stats in the 'categories' structure like mlb_player_gamelog.py does
        ...
        for category in season_type.get('categories', []):
            if category.get('type') == 'event':
                ...
                for event in category.get('events', []):
                    ...
                    # Get opponent - first try from event_details, then from event
                    ...
                    # Extract result from event_details if available
                    game_result = 'N/A'
                    if event_details:
...
# Process season summary stats (if available)
summary_stats = []

# Method 1: Try to get summary from the data.summary (like in gamelog_test.py)
if 'summary' in data and 'stats' in data['summary']:
    summary_stats = data['summary'].get('stats', [])

# Method 2: Try to get summary from seasonTypes (like in mlb_player_gamelog.py)
if not summary_stats and 'seasonTypes' in data:
    for season_type in data.get('seasonTypes', []):
