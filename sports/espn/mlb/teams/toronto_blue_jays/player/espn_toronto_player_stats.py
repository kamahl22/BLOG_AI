...
def scrape_blue_jays_player_stats():
    print("📡 Fetching Toronto Blue Jays player stats...")
        ...
        try:
            ...
            for table in tables:
                ...
                if rows:
                    ...
                    # client.table("player_stats").insert([{"team": "toronto-blue-jays", "player": player['name'], **dict(zip(headers, row))} for row in rows]).execute()
        except Exception as e:
...
if __name__ == "__main__":
    stats = scrape_blue_jays_player_stats()
