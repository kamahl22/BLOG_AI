...
def fetch_player_news():
    ...
    if response.status_code != 200:
        print(f"Error fetching news for Caleb Thielbar: {response.status_code}")
        return
    soup = BeautifulSoup(response.text, "html.parser")
    articles = soup.find_all("article")
    if not articles:
        print(f"No news found for Caleb Thielbar")
        return
    with open(os.path.join("/Users/kamahl/BLOG_AI/database/espn/mlb/teams/chicago_cubs/player/player/caleb_thielbar", "caleb_thielbar_news.csv"), "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"Saved news for Caleb Thielbar")
