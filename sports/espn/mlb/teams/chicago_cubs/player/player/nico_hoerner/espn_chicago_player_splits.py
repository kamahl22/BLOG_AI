...
def fetch_player_splits():
    ...
    if response.status_code != 200:
        print(f"Error fetching splits for Nico Hoerner: {response.status_code}")
        return
    soup = BeautifulSoup(response.text, "html.parser")
    tables = soup.find_all("table")
    if not tables:
        print(f"No splits tables found for Nico Hoerner")
        return
    with open(os.path.join("/Users/kamahl/BLOG_AI/database/espn/mlb/teams/chicago_cubs/player/player/nico_hoerner", "nico_hoerner_splits.csv"), "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"Saved splits for Nico Hoerner")
