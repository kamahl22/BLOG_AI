...
def fetch_player_news():
    ...
    if response.status_code != 200:
        print(f"Error fetching news for Jameson Taillon: {response.status_code}")
        return
    soup = BeautifulSoup(response.text, "html.parser")
    articles = soup.find_all("article")
    if not articles:
        print(f"No news found for Jameson Taillon")
        return
    with open(os.path.join("/Users/kamahl/BLOG_AI/database/espn/mlb/teams/chicago_cubs/player/player/jameson_taillon", "jameson_taillon_news.csv"), "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"Saved news for Jameson Taillon")
