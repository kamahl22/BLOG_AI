...
def scrape_cubs_batter_vs_pitcher():
    print("📡 Fetching Chicago Cubs batter vs. pitcher data...")
        ...
        try:
            ...
            for table in tables:
                ...
                if rows:
                    ...
                    # client.table("batter_vs_pitcher").insert([{"team": "chicago-cubs", "player": player['name'], **dict(zip(headers, row))} for row in rows]).execute()
        except Exception as e:
...
if __name__ == "__main__":
    bvp = scrape_cubs_batter_vs_pitcher()
