...
url = f"https://www.espn.com/mlb/player/batvspitch/_/id/2560565/teamId/4"
...
try:
    ...
    print(f"Error fetching bat vs pitch for Cam Booser (ID: 2560565): {e}")
    exit(1)
...
if not tables:
    print(f"No bat vs pitch tables found for Cam Booser")
    exit(1)
...
if df.empty:
    print(f"No bat vs pitch data found for Cam Booser")
    exit(1)
...
output_file = os.path.join(output_dir, f"cam_booser_batvspitch_2025_{datetime.now().strftime('%Y%m%d')}.csv")
...
print(f"\nCam Booser 2025 Bat vs Pitch Data:")
