...
player_name_slug = "shane-baz"

# URL for stats
url = f"https://www.espn.com/mlb/player/stats/_/id/39639/shane-baz"
...
try:
    ...
    print(f"Error fetching stats for Shane Baz (ID: 39639): {e}")
    exit(1)
...
if not tables:
    print(f"No stats tables found for Shane Baz")
    exit(1)
...
if df.empty:
    print(f"No stats data found for Shane Baz")
    exit(1)
...
output_file = os.path.join(output_dir, f"shane_baz_stats_2025_{datetime.now().strftime('%Y%m%d')}.csv")
