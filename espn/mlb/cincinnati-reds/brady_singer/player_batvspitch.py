...
url = f"https://www.espn.com/mlb/player/batvspitch/_/id/41172/teamId/17"
...
try:
    ...
    print(f"Error fetching bat vs pitch for Brady Singer (ID: 41172): {e}")
    exit(1)
...
if not tables:
    print(f"No bat vs pitch tables found for Brady Singer")
    exit(1)
...
if df.empty:
    print(f"No bat vs pitch data found for Brady Singer")
    exit(1)
...
output_file = os.path.join(output_dir, f"brady_singer_batvspitch_2025_{datetime.now().strftime('%Y%m%d')}.csv")
...
print(f"\nBrady Singer 2025 Bat vs Pitch Data:")
