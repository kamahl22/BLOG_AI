...
url = f"https://www.espn.com/mlb/player/batvspitch/_/id/40423/teamId/17"
...
try:
    ...
    print(f"Error fetching bat vs pitch for Gavin Lux (ID: 40423): {e}")
    exit(1)
...
if not tables:
    print(f"No bat vs pitch tables found for Gavin Lux")
    exit(1)
...
if df.empty:
    print(f"No bat vs pitch data found for Gavin Lux")
    exit(1)
...
output_file = os.path.join(output_dir, f"gavin_lux_batvspitch_2025_{datetime.now().strftime('%Y%m%d')}.csv")
...
print(f"\nGavin Lux 2025 Bat vs Pitch Data:")
