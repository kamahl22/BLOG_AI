...
player_name_slug = "jacob-webb"

# URL for stats
url = f"https://www.espn.com/mlb/player/stats/_/id/40384/jacob-webb"
...
try:
    ...
    print(f"Error fetching stats for Jacob Webb (ID: 40384): {e}")
    exit(1)
...
if not tables:
    print(f"No stats tables found for Jacob Webb")
    exit(1)
...
if df.empty:
    print(f"No stats data found for Jacob Webb")
    exit(1)
...
output_file = os.path.join(output_dir, f"jacob_webb_stats_2025_{datetime.now().strftime('%Y%m%d')}.csv")
