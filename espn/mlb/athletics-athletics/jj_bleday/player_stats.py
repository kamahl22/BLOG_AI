...
player_name_slug = "jj-bleday"

# URL for stats
url = f"https://www.espn.com/mlb/player/stats/_/id/42410/jj-bleday"
...
try:
    ...
    print(f"Error fetching stats for JJ Bleday (ID: 42410): {e}")
    exit(1)
...
if not tables:
    print(f"No stats tables found for JJ Bleday")
    exit(1)
...
if df.empty:
    print(f"No stats data found for JJ Bleday")
    exit(1)
...
output_file = os.path.join(output_dir, f"jj_bleday_stats_2025_{datetime.now().strftime('%Y%m%d')}.csv")
