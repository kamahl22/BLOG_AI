...
player_name_slug = "shea-langeliers"

# URL for stats
url = f"https://www.espn.com/mlb/player/stats/_/id/42598/shea-langeliers"
...
try:
    ...
    print(f"Error fetching stats for Shea Langeliers (ID: 42598): {e}")
    exit(1)
...
if not tables:
    print(f"No stats tables found for Shea Langeliers")
    exit(1)
...
if df.empty:
    print(f"No stats data found for Shea Langeliers")
    exit(1)
...
output_file = os.path.join(output_dir, f"shea_langeliers_stats_2025_{datetime.now().strftime('%Y%m%d')}.csv")
...
print(f"\nShea Langeliers 2025 Stats Data:")
