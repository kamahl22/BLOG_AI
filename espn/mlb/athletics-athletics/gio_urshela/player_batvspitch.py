...
url = f"https://www.espn.com/mlb/player/batvspitch/_/id/32721/teamId/11"
...
try:
    ...
    print(f"Error fetching bat vs pitch for Gio Urshela (ID: 32721): {e}")
    exit(1)
...
if not tables:
    print(f"No bat vs pitch tables found for Gio Urshela")
    exit(1)
...
if df.empty:
    print(f"No bat vs pitch data found for Gio Urshela")
    exit(1)
...
output_file = os.path.join(output_dir, f"gio_urshela_batvspitch_2025_{datetime.now().strftime('%Y%m%d')}.csv")
...
print(f"\nGio Urshela 2025 Bat vs Pitch Data:")
