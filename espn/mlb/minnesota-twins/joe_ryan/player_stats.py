...
player_name_slug = "joe-ryan"

# URL for stats
url = f"https://www.espn.com/mlb/player/stats/_/id/42450/joe-ryan"
...
try:
    ...
    print(f"Error fetching stats for Joe Ryan (ID: 42450): {e}")
    exit(1)
...
if not tables:
    print(f"No stats tables found for Joe Ryan")
    exit(1)
...
if df.empty:
    print(f"No stats data found for Joe Ryan")
    exit(1)
...
output_file = os.path.join(output_dir, f"joe_ryan_stats_2025_{datetime.now().strftime('%Y%m%d')}.csv")
