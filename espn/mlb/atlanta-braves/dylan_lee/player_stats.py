...
player_name_slug = "dylan-lee"

# URL for stats
url = f"https://www.espn.com/mlb/player/stats/_/id/41455/dylan-lee"
...
try:
    ...
    print(f"Error fetching stats for Dylan Lee (ID: 41455): {e}")
    exit(1)
...
if not tables:
    print(f"No stats tables found for Dylan Lee")
    exit(1)
...
if df.empty:
    print(f"No stats data found for Dylan Lee")
    exit(1)
...
output_file = os.path.join(output_dir, f"dylan_lee_stats_2025_{datetime.now().strftime('%Y%m%d')}.csv")
