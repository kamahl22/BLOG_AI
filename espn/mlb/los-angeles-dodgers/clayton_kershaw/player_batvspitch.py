...
url = f"https://www.espn.com/mlb/player/batvspitch/_/id/28963/teamId/19"
...
try:
    ...
    print(f"Error fetching bat vs pitch for Clayton Kershaw (ID: 28963): {e}")
    exit(1)
...
if not tables:
    print(f"No bat vs pitch tables found for Clayton Kershaw")
    exit(1)
...
if df.empty:
    print(f"No bat vs pitch data found for Clayton Kershaw")
    exit(1)
...
output_file = os.path.join(output_dir, f"clayton_kershaw_batvspitch_2025_{datetime.now().strftime('%Y%m%d')}.csv")
...
print(f"\nClayton Kershaw 2025 Bat vs Pitch Data:")
