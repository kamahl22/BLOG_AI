...
player_name_slug = "bo-naylor"

# URL for stats
url = f"https://www.espn.com/mlb/player/stats/_/id/41183/bo-naylor"
...
try:
    ...
    print(f"Error fetching stats for Bo Naylor (ID: 41183): {e}")
    exit(1)
...
if not tables:
    print(f"No stats tables found for Bo Naylor")
    exit(1)
...
if df.empty:
    print(f"No stats data found for Bo Naylor")
    exit(1)
...
output_file = os.path.join(output_dir, f"bo_naylor_stats_2025_{datetime.now().strftime('%Y%m%d')}.csv")
