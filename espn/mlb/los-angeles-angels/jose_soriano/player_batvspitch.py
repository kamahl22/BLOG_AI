...
url = f"https://www.espn.com/mlb/player/batvspitch/_/id/40973/teamId/3"
...
try:
    ...
    print(f"Error fetching bat vs pitch for Jose Soriano (ID: 40973): {e}")
    exit(1)
...
if not tables:
    print(f"No bat vs pitch tables found for Jose Soriano")
    exit(1)
...
if df.empty:
    print(f"No bat vs pitch data found for Jose Soriano")
    exit(1)
...
output_file = os.path.join(output_dir, f"jose_soriano_batvspitch_2025_{datetime.now().strftime('%Y%m%d')}.csv")
...
print(f"\nJose Soriano 2025 Bat vs Pitch Data:")
