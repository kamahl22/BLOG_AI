...
url = f"https://www.espn.com/mlb/player/batvspitch/_/id/36040/teamId/29"
...
try:
    ...
    print(f"Error fetching bat vs pitch for Lourdes Gurriel Jr. (ID: 36040): {e}")
    exit(1)
...
if not tables:
    print(f"No bat vs pitch tables found for Lourdes Gurriel Jr.")
    exit(1)
...
if df.empty:
    print(f"No bat vs pitch data found for Lourdes Gurriel Jr.")
    exit(1)
...
output_file = os.path.join(output_dir, f"lourdes_gurriel_jr_batvspitch_2025_{datetime.now().strftime('%Y%m%d')}.csv")
...
print(f"\nLourdes Gurriel Jr. 2025 Bat vs Pitch Data:")
