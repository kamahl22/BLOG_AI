...
def scrape_cubs_news():
    print("📡 Fetching Chicago Cubs news...")
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    chrome_options.add_argument("--disable-gpu")
    driver = webdriver.Chrome(options=chrome_options)
    try:
        driver.get("https://www.espn.com/mlb/team/_/name/chc/chicago-cubs")
        time.sleep(3)
        ...
        if all_data:
            print("Chicago Cubs News:")
            print(tabulate(all_data, headers=["Headline", "Link"], tablefmt="grid"))
            # client.table("team_news").insert([{"team": "chicago-cubs", "headline": row[0], "link": row[1]} for row in all_data]).execute()
        return all_data
