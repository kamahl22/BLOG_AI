...
def fetch_player_stats():
    ...
    if response.status_code != 200:
        print(f"Error fetching stats for Nicky Lopez: {response.status_code}")
        return
    soup = BeautifulSoup(response.text, "html.parser")
    tables = soup.find_all("table")
    if not tables:
        print(f"No stats tables found for Nicky Lopez")
        return
    with open(os.path.join("/Users/kamahl/BLOG_AI/database/espn/mlb/teams/chicago_cubs/player/player/nicky_lopez", "nicky_lopez_stats.csv"), "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"Saved stats for Nicky Lopez")
