...
def fetch_player_splits():
    ...
    if response.status_code != 200:
        print(f"Error fetching splits for Matthew Boyd: {response.status_code}")
        return
    soup = BeautifulSoup(response.text, "html.parser")
    tables = soup.find_all("table")
    if not tables:
        print(f"No splits tables found for Matthew Boyd")
        return
    with open(os.path.join("/Users/kamahl/BLOG_AI/database/espn/mlb/teams/chicago_cubs/player/player/matthew_boyd", "matthew_boyd_splits.csv"), "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"Saved splits for Matthew Boyd")
