...
def fetch_batter_vs_pitcher():
    ...
    if response.status_code != 200:
        print(f"Error fetching batter vs pitcher for Chris Flexen: {response.status_code}")
        return
    soup = BeautifulSoup(response.text, "html.parser")
    tables = soup.find_all("table")
    if not tables:
        print(f"No batter vs pitcher data found for Chris Flexen")
        return
    with open(os.path.join("/Users/kamahl/BLOG_AI/database/espn/mlb/teams/chicago_cubs/player/player/chris_flexen", "chris_flexen_batter_vs_pitcher.csv"), "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print(f"Saved batter vs pitcher for Chris Flexen")
