...
def scrape_blue_jays_injuries():
    print("📡 Fetching Toronto Blue Jays injuries...")
    chrome_options = Options()
    chrome_options.add_argument("--headless")
    chrome_options.add_argument("--disable-gpu")
    driver = webdriver.Chrome(options=chrome_options)
    try:
        driver.get("https://www.espn.com/mlb/team/injuries/_/name/tor/toronto-blue-jays")
        time.sleep(3)
        ...
        for tr in table.find("tbody").find_all("tr")[:5]:  # Limit to 5 injuries
            ...
            if row and "Out" in row[1]:  # Only active injuries
                rows.append(row)
        if rows:
            print("Toronto Blue Jays Injuries:")
            print(tabulate(rows, headers=headers, tablefmt="grid"))
            # client.table("team_injuries").insert([{"team": "toronto-blue-jays", **dict(zip(headers, row))} for row in rows]).execute()
        return rows
...
if __name__ == "__main__":
    injuries = scrape_blue_jays_injuries()
