...
def scrape_blue_jays_roster():
    print("📡 Fetching Toronto Blue Jays roster...")
    ...
    try:
        ...
        for i, table in enumerate(tables):
            ...
            for tr in table.find("tbody").find_all("tr"):
                ...
                row_dict = {"team": "toronto-blue-jays"}
        ...
        if all_rows:
            print("Toronto Blue Jays Roster:")
            print(tabulate([list(r.values()) for r in all_rows], 
                          headers=list(all_rows[0].keys()), 
                          tablefmt="grid"))
            try:
                # Clear old Blue Jays data to ensure latest roster (handles traded players)
                client.table("team_roster").delete().eq("team", "toronto-blue-jays").execute()
                # Insert new data
...
if __name__ == "__main__":
    roster = scrape_blue_jays_roster()
